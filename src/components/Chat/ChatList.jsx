...
/**
 * ChatList — left panel of the chat page.
 *
 * CHANGED: The "New Message" panel now shows ALL users (not just shared-book contacts).
 *          The empty-state message updated accordingly.
 */
const ChatList = ({ chats, selectedChatId, loading, onSelectChat, onChatsUpdated }) => {
  const { user } = useAuth();
  const [showNewChat, setShowNewChat] = useState(false);
  ...
  const openNewChat = async () => {
    ...
    try {
      ...
      const existingChatUserIds = chats.map(c => c.otherUser._id);

      // Filter out users we already have chats with
      const filteredUsers = allUsers.filter(u => !existingChatUserIds.includes(u._id));

      setChatableUsers(filteredUsers);
    } catch (err) {
      console.error('Failed to load users:', err);
    } finally {
      setLoadingUsers(false);
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="text-center py-10 px-4">
            ...
            <p className="text-sm text-gray-500 font-medium">No conversations yet</p>
            <p className="text-xs text-gray-400 mt-1">Click + to start chatting with any user</p>
          </div>
        ) : (
          chats.map((chat) => (
            <button
              key={chat._id}
              onClick={() => onSelectChat(chat)}
              className={`w-full flex items-center px-4 py-3 transition-colors border-b border-gray-50 ${selectedChatId === chat._id ? 'bg-blue-50' : 'hover:bg-gray-50'
                }`}
            >
              <div className="relative flex-shrink-0">
                <Avatar user={chat.otherUser} />
