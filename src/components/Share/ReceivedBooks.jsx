...
const ReceivedBooks = () => {
  ...
  const fetchSharedBooks = async () => {
    ...
    if (!user?._id) {
      console.log('ReceivedBooks: Waiting for user to load...');
      setLoading(false);
      return;
    }
    ...
    try {
      setLoading(true);
      console.log('ReceivedBooks: Fetching received books for user:', user._id);
      console.log('ReceivedBooks: API endpoint:', `/users/${user._id}/received-books`);
      
      const response = await shareService.getReceivedBooks(user._id);
      console.log('ReceivedBooks: Full response:', response);
      
      // FIXED: Handle the correct response format from backend
      // Backend returns { message: "...", receivedBooks: [...] }
      const booksData = response.receivedBooks || response.books || response.data || response || [];
      console.log('ReceivedBooks: Extracted books data:', booksData);
      console.log('ReceivedBooks: Number of received books:', Array.isArray(booksData) ? booksData.length : 0);
      
      setBooks(Array.isArray(booksData) ? booksData : []);
    } catch (error) {
      console.error('ReceivedBooks: Error fetching received books:', error);
      console.error('ReceivedBooks: Error response:', error.response);
      console.error('ReceivedBooks: Error data:', error.response?.data);
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-lg shadow-md p-12 text-center">
              ...
              <p className="text-gray-600">
                No one has shared any books with you yet.
              </p>
            </div>
          ) : (
